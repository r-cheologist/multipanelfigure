...
When you add a plot or image to a multi-panel figure, you can specify which panels it should cover.  The `add_panel` function will automatically choose the next free location for you if you don't specify it.
...
If we don't specify which panel to fill, `add_panel` defaults to using the top-left panel.

```{r, panel1_1}
(figure %<>% add_panel(rect_grob()))
```
...
If you want to cover multiple panels, or specify a different position, you can manually set the `top_panel`, `bottom_panel`, `left_panel`, and  `right_panel` arguments.  In the following example, `top_panel` and `bottom_panel` are set so that the rectangle covers rows one to three. The `left_panel` argument specifies that the third column should be used; since `right_panel` is not specified, the image only covers a single column.

```{r, panel1.3_3}
(figure %<>% add_panel(rect_grob(), top_panel = 1, bottom_panel = 3, left_panel = 3))
```
...
```{r, panel2.3_2.3}
(figure %<>% add_panel(rect_grob(), left_panel = 2, right_panel = 3, bottom_panel = 3))
```
...
(figure %<>% add_panel(rect_grob(), left_panel = 2, right_panel = 3, bottom_panel = 3, allow_panel_overwriting = TRUE))
```

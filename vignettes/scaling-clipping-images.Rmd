...
(figure %<>% add_panel(r_logo, scaling = "stretch"))
```
...
(figure %<>% add_panel(r_logo, scaling = "fit"))
```
...
(figure %<>% add_panel(r_logo, scaling = "shrink", top_panel = 3, left_panel = 1))
```

If the image is smaller than the panel, `scaling = "shrink"` works like `scaling = "none"`.

```{r, shrink_big, fig.height = 8, fig.width = 8}
(figure %<>% add_panel(r_logo, scaling = "shrink", top_panel = 2, bottom_panel = 3, left_panel = 2))
```
...
figure2 <- multi_panel_figure(
  widths = rep.int(1.25, 3),
  heights = rep.int(1.25, 3),
  unit = "inches")
...
for(position in list(c(1, 1), c(1, 2), c(1, 3), c(2, 1), c(2, 3), c(3, 1), c(3, 2), c(3, 3)))
{
  figure2 %<>% 
    add_panel(
      ...
      top_panel = position[1], 
      left_panel = position[2])
}
figure2
```
...
(figure2 %>% add_panel(rhino, panel_clip = "off"))
```
